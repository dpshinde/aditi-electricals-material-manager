...
export default function GodownDetail() {
  const { godownId } = useParams({ from: "/godowns/$godownId" });
  ...
  const godown = godowns.find((g) => g.id.toString() === godownId);
  if (!godown)
    return <div className="p-6 text-muted-foreground">Godown not found.</div>;
  ...
  const materialSummary = stockItems.map((item) => {
    ...
    for (const m of movements) {
      ...
      if (
        m.source &&
        m.source.id === godown.id &&
        m.source.type === LocationType.godown
      ) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-foreground">{godown.name}</h1>
          {godown.location && (
            <p className="text-sm text-muted-foreground">{godown.location}</p>
          )}
        </div>
      </div>
      ...
      <div className="rounded-xl border overflow-hidden">
        ...
        {showForm && (
          <div className="p-4">
            <MovementForm
              defaultToType="godown"
              defaultToLocationId={godown.id}
